...
public class IndexerCURD {
    ...
    //索引存储地址
    private static String indexDir = "E:\\study\\indexerCRUD";
    ...
    public void TestSearchaer() {
        try {
            ...
            QueryParser parser = new QueryParser("describes", getAnalyzer());//指定Document的某个属性
            //Query query = parser.parse("kl");//指定索引内容，对应某个分词
            //Query query = parser.parse("wn");//指定索引内容，对应某个分词
            Query query = parser.parse("know");
            Term term = new Term("names", "kl");
            //参数是一个选项，可以是一个query，也可以是一个term，term是一个精确查找的值
